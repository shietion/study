...
public class GeneratorRest {
     ...
     * rest 服务模板
     */
    private static String resource_vm = "/template/Resource.vm";
	...
	generatorConfig_vm = GeneratorRest.class.getResource(generatorConfig_vm).getPath().replaceFirst("/", "");
	service_vm = GeneratorRest.class.getResource(service_vm).getPath().replaceFirst("/", "");
	serviceImpl_vm = GeneratorRest.class.getResource(serviceImpl_vm).getPath().replaceFirst("/", "");

	String basePath = GeneratorRest.class.getResource("/").getPath().replace("/target/classes/", "").replace(targetProject, "").replaceFirst("/", "");
	String generatorConfig_xml = GeneratorRest.class.getResource("/").getPath().replace("/target/classes/", "") + "/src/main/resources/generatorConfig.xml";
	...
	String controllerPath = targetProject + "/src/main/java/" + package_name.replaceAll("\\.", "/") + "/resource";
	...
	for (int i = 0; i < tables.size(); i++) {
	    String model = lineToHump(ObjectUtils.toString(tables.get(i).get("table_name")));
	    String controller = controllerPath + "/" + model + "Resource.java";
	    // 生成service
		...
		context.put("model", model);
		context.put("ctime", ctime);
		VelocityUtil.generate(resource_vm, controller, context);
